...
getDensities <- function(files,
    ...
    for(file in files){
        ...
        for (channel in channels) {
            ...
            quant <- quantile(ff@exprs[, channel],
                                     quantileValues)
...
#' 
#' @importFrom stats sd
#' @importFrom FlowSOM GetClusters metaClustering_consensus
#'
#' @examples
#'
#' dir <- system.file("extdata", package = "CytoNorm")
  ...
  FlowSOM nodes (", nClus, ")."))
    ...
    for(mc in cluster_values){
        ...
        meta_cl[[as.character(mc)]] <-
            metaClustering_consensus(fsom$map$codes,
                                              mc,
                                              seed = seed)
    }
...
PlotOverviewCV <- function(fsom, cv_res, max_cv = 2.5, show_cv = 1.5){
    ...
    p_pctgs_selected <- pheatmap(pctgs[[as.character(nMetaClus)]],
                                           cluster_cols = FALSE,
                                           cluster_rows = FALSE,
                                           na_col = "white",
                                           border_color = "white",
    ...
    grid.arrange(p_cvs[[4]], p_pctgs_selected[[4]])
}
...
#'                  Default = 2 
#' 
#' @importFrom flowCore read.FCS transform exprs
#' @importFrom graphics hist
#' @importFrom FlowSOM GetMarkers
...
emdEvaluation <- function(files,
    ...
    for (file in files) {
        ...
        if (is.null(prettyColnames)){
            prettyColnames <- paste0(GetMarkers(ff, channels = channels),
                                     " <", channels, ">")
        }
        ...
        for (cellType in cellTypes) {
            ...
            if (sum(selection) >= manualThreshold){
                distr[[file]][[cellType]] <-
                    apply(exprs(ff)[selection, channels],
                          2,
                          function(x) {
                              hist(x,
            ...
            for (i in seq_along(files)[-length(files)]) {
                ...
                for (j in seq(i + 1, length(files))) {
                    ...
                    distances[[cellType]][[name]][file1, file2] <-
                        emd2d(matrix(distr[[file1]][[cellType]][, name]),
                           ...
                           return_all = FALSE)
{
    ...
    for (file in files) {
        ...
        for (cellType in cellTypes) {
            ...
            if (sum(selection) > 1){
                mad_values[[file]][[cellType]] <- apply(exprs(ff)[selection,
